...
if(isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['producto']) && !empty($_POST['producto']) && isset($_POST['precio']) && !empty($_POST['precio']))
{
	...
	mysql_query("UPDATE populares SET producto='$_POST[producto]',precio='$_POST[precio]' WHERE id=$_POST[id]",$con) or die (mysql_error());
    echo '<script type="text/javascript"> window.location="principalpo.php"</script>';

}
else{
	echo "PROBLEMAS AL MODIFICAR DATOS";
	echo '<script type="text/javascript"> window.location="formmodpo.php"</script>';
}
?>